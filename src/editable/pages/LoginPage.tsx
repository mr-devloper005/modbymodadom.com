      ...
      <main>
        <section className="mx-auto grid min-h-[calc(100vh-12rem)] max-w-[var(--editable-container)] items-center gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[1fr_0.9fr] lg:px-8">
          <div>
            ...
            <h1 className="editable-display mt-4 max-w-xl text-5xl font-semibold leading-[0.96] tracking-[-0.05em] text-[var(--slot4-cream)] sm:text-6xl">{pagesContent.auth.login.title}</h1>
          ...
          <div className="rounded-[2rem] border border-[var(--editable-border)] bg-white/4 p-7 shadow-[0_20px_48px_rgba(0,0,0,0.18)] sm:p-9">
            <h2 className="editable-display text-2xl font-semibold text-[var(--slot4-cream)]">{pagesContent.auth.login.formTitle}</h2>
            <EditableLocalLoginForm />
            <p className="mt-6 text-sm text-[var(--slot4-muted-text)]">New here? <Link href="/signup" className="font-semibold text-[var(--slot4-page-text)] underline-offset-4 hover:underline">{pagesContent.auth.login.createCta}</Link></p>
          </div>
        </section>
      </main>
    </EditableSiteShell>

...
const inputClass =
  'h-12 w-full rounded-2xl border border-[var(--editable-border)] bg-white/5 px-4 text-sm font-medium text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-muted-text)] focus:border-[var(--slot4-accent)]'
const buttonClass =
  'inline-flex h-12 w-full items-center justify-center rounded-full bg-[var(--editable-cta-bg)] px-6 text-sm font-semibold text-[var(--editable-cta-text)] transition hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? (
        <p className={`rounded-2xl px-4 py-3 text-sm font-medium ${status === 'success' ? 'bg-emerald-950/30 text-emerald-200' : 'bg-[var(--slot4-accent-soft)] text-[var(--slot4-page-text)]'}`}>
          {message}
        </p>
      ) : null}

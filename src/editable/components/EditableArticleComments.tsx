...
export function EditableArticleComments({ slug, comments = [] }: { slug: string; comments?: Comment[] }) {
  ...
  return (
    <section className="mt-14 rounded-[2rem] border border-[var(--tk-line)] bg-white/4 p-6 pt-8 sm:p-8">
      <div className="flex items-center gap-2 text-lg font-semibold text-[var(--tk-text)]">
        <MessageCircle className="h-5 w-5 text-[var(--tk-accent)]" /> Comments
        <span className="text-[var(--tk-muted)]">({all.length})</span>
      </div>

      <form onSubmit={submit} className="mt-6 rounded-[1.5rem] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-5">
        <input
          value={name}
          onChange={(event) => setName(event.target.value)}
          placeholder="Your name (optional)"
          maxLength={60}
          className="h-11 w-full rounded-xl border border-[var(--tk-line)] bg-[var(--tk-bg)] px-4 text-sm outline-none transition focus:border-[var(--tk-accent)]"
        />
        <textarea
          value={text}
          ...
          rows={4}
          maxLength={1500}
          className="mt-3 w-full resize-y rounded-xl border border-[var(--tk-line)] bg-[var(--tk-bg)] px-4 py-3 text-sm leading-6 outline-none transition focus:border-[var(--tk-accent)]"
        />
        <div className="mt-3 flex justify-end">
          <button
            type="submit"
            disabled={!text.trim()}
            className="inline-flex items-center gap-2 rounded-full bg-[var(--tk-accent)] px-6 py-2.5 text-sm font-semibold text-[var(--tk-on-accent)] transition hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-50"
          >
